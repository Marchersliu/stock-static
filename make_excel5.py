...
# ===== 时间轴：每个时间段一个单元格，等宽 =====
timeline = [
    ('date', date(2026, 4, 29), '04/29', '周二'),
    ('date', date(2026, 4, 30), '04/30', '周三'),
    ('holiday', [date(2026, 5, 1), date(2026, 5, 5)], '05/01\n~\n05/05', 'Holiday'),
    ('date', date(2026, 5, 6), '05/06', '周二'),
    ('date', date(2026, 5, 7), '05/07', '周三'),
    ('date', date(2026, 5, 8), '05/08', '周四'),
    ('date', date(2026, 5, 9), '05/09', '周五'),
    ('date', date(2026, 5, 10), '05/10', '周六'),
    ('date', date(2026, 5, 11), '05/11', '周日'),
    ('date', date(2026, 5, 12), '05/12', '周一'),
    ('blank', [date(2026, 5, 13), date(2026, 5, 23)], '...', ''),
    ('date', date(2026, 5, 24), '05/24', '周日'),
    ('blank', [date(2026, 5, 25), date(2026, 5, 28)], '...', ''),
    ('date', date(2026, 5, 29), '05/29', '周四'),
]
...
col_count = len(timeline) + 1  # +1 for Substrate column
...
for i, (seg_type, _, date_text, _) in enumerate(timeline, 2):
    cell = ws.cell(row=2, column=i, value=date_text)
    cell.border = thin_border
    cell.alignment = center_align
    if seg_type == 'date':
        cell.fill = header_fill
        cell.font = header_font
    elif seg_type == 'holiday':
        cell.fill = holiday_fill
        cell.font = Font(name='微软雅黑', size=9, italic=True, color="78909C")
    else:
...
for i, (seg_type, _, _, wd_text) in enumerate(timeline, 2):
    cell = ws.cell(row=3, column=i, value=wd_text)
    cell.border = thin_border
    cell.alignment = center_align
    if seg_type == 'date':
        cell.fill = sub_fill
...
for i, (seg_type, dates, _, _) in enumerate(timeline, 2):
